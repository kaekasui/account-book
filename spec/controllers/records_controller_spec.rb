  ...
  describe "POST create" do
    describe "with valid params" do
      it "creates a new Record" do
        expect {
          post :create, { record: attributes_for(:record, breakdown_id: breakdown.id) }
  ...
  describe "PUT update" do
    describe "with valid params" do
      ...
      it "updates the requested record" do
        record = create(:record, breakdown_id: breakdown.id)
        put :update, { id: record.id, record: attributes_for(:record, charge: 2000, breakdown_id: breakdown.id) }
        record.reload
      ...
      it "assigns the requested record as @record" do
        record = create(:record, breakdown_id: breakdown.id)
        put :update, { id: record.id, record: attributes_for(:record, breakdown_id: breakdown.id) }
        expect(assigns(:record)).to eq(record)
      end
